...
//       1         2         3         4         5         6         7         8
// Filename: ../engine_juliet.cpp
// Last Modified: Wed Jul 01, 2020  02:32PM
// LOC: 1017
// Filesize: 48078 bytes
//  
...
//
// Note on graphics: These are the layers:
//
//     1. background (solid color)
//     2. sprite layer A
//     3. sprite layer B
//     4. textgrid background (60x45)
//     5. textgrid foreground (60x45)
//     6. keyboard cursor
//     7. mouse cursor
//     8. gamepad cursor
//
// Note on text data: the standard ASCII set is used (128 codes), but only 98
// codes have actual glyphs (codes 33-126 and 17-20 for arrow glyphs). The 
// textgrid arrays hold 'char' data, which runs -128 to 127. 
//
// Valid gamepad constants (from the SDL library itself):
//
...
//
// Valid mouse constants (from the SDL library):
//
//     SDL_BUTTON_LEFT
//     SDL_BUTTON_MIDDLE
//     SDL_BUTTON_RIGHT
...
// GAMEPAD (no cursor available) 
SDL_GameController*   gamepad;
bool gamepad_enabled = false;
bool gamepad_cursor_enabled = false;
int gamepad_cursor_x = 0;    // textgrid (cell) coordinates: 60 x 45
int gamepad_cursor_y = 0;          

// MOUSE (mouse_cursor available and optional, default is off)
bool mouse_enabled = false;
bool mouse_cursor_enabled = false;
int mouse_cursor_x = 0;    // textgrid (cell) coordinates: 60 x 45
int mouse_cursor_y = 0;          

//KEYBOARD (keyboard_cursor available and optional, default is off)
bool keyboard_enabled = true;
bool keyboard_cursor_enabled = false;  
int keyboard_cursor_x = 0;     // textgrid (cell) coordinates: 60 x 45          
...
//INPUT (ARRAYS OF FUNCTION POINTERS)
void (*action_pointer_gamepad[ NUMBER_OF_GAMEPAD_INPUTS ]) (int action);
void (*action_pointer_mouse[ NUMBER_OF_MOUSE_INPUTS ]) (int action);
void (*action_pointer_keyboard[ NUMBER_OF_KEYBOARD_INPUTS ]) (int action);

//dummy input handler (for array of function pointers that deal with input)
void dummy_game_action(int action) {
}
...
SDL_AudioFormat     audio_format = AUDIO_U8; // 0 to 255, 8-bit sound
int                 audio_frequency = 44100; // hearing range: 120 Hz to 11000 Hz
...
Uint32 target_loop_duration = (1000 / DESIRED_FPS) - 1; 
...
char*     ENGINE_NAME = "ENGINE JULIET";
...
int main_game_loop() {
    ...
    while(quit_program == false) {
        ...
        while(SDL_PollEvent(&input_event) != 0) {
            ...
            switch(input_event.type) {
                ...
                case SDL_CONTROLLERBUTTONDOWN:
                    if(gamepad_enabled == true) {
                        (*action_pointer_gamepad[input_event.cbutton.button]) (START_ACTION);
                    }
                    break;
                case SDL_CONTROLLERBUTTONUP:
                    if(gamepad_enabled == true) {
                        (*action_pointer_gamepad[input_event.cbutton.button]) (STOP_ACTION);
                    }
                    break;
                ...
                case SDL_MOUSEMOTION:
                    if(mouse_enabled == true) {
                        if(current_graphics_mode == FULLSCREEN_MODE) {
                            user_mouse_motion_handler(
                                (input_event.motion.x / current_scale_factor) - target_texture_rect.x, 
                                (input_event.motion.y / current_scale_factor) - target_texture_rect.y);
                            if(mouse_cursor_enabled == true) {
                                mouse_cursor_x = (input_event.motion.x / current_scale_factor) 
                                                    - target_texture_rect.x;
                                mouse_cursor_y = (input_event.motion.y / current_scale_factor)
                ...
                case SDL_MOUSEBUTTONDOWN:
                    if(mouse_enabled == true) {
                        (*action_pointer_mouse[input_event.button.button]) (START_ACTION);
                    }
                    break;
                case SDL_MOUSEBUTTONUP:
                    if(mouse_enabled == true) {
                        (*action_pointer_mouse[input_event.button.button]) (STOP_ACTION);
                    }
                    break;
        ...
        if(mouse_cursor_enabled) {
            ...
            SDL_RenderFillRect(window_renderer, 
                    &text_rect[(int)(mouse_cursor_y/FONT_HEIGHT)]
                              [(int)(mouse_cursor_x/FONT_WIDTH)]);
        } 
...
bool initialize_engine() {
    ...
    //Point every key/button input to dummy_game_action function
    for(int i = 0; i < 285; i++) {
        action_pointer_keyboard[i] = dummy_game_action;
    }
    for(int i = 0; i < 10; i++) {
        action_pointer_mouse[i] = dummy_game_action;
    }
    for(int i = 0; i < 30; i++) {
        action_pointer_gamepad[i] = dummy_game_action;
    }
...
void initialize_audio(void) {
    ...
    printf(" SOUND: chosen audio device id: %d\n", audio_device_id);
    fflush(stdout);

    printf(" SOUND: available frequency: %d\n", have.freq);
    printf(" SOUND: available format: %d\n", have.format);
    printf(" SOUND: available channels: %d\n", have.channels);
    printf(" SOUND: available samples: %d\n", have.samples);
    fflush(stdout);
}
...
bool build_window_and_renderer() {
    ...
    if(current_graphics_mode == FULLSCREEN_MODE) {
        ...
        window = SDL_CreateWindow(
                "SDL2/C++ Game Engine 2020 (JULIET)",
    ...
    if(window == NULL) {
        ...
        if(current_graphics_mode == FULLSCREEN_MODE) {
            ...
            //Make sure to update global variable tracking scale factor
...
void load_wav_sound_file(const char *filename, int i) {
...
void keyboard_key_up_handler(SDL_Event e) {
    ...
    // Call function that has been set up by user
    (*action_pointer_keyboard[e.key.keysym.scancode]) (STOP_ACTION);
}
...
void keyboard_key_down_handler(SDL_Event e) {
    ...
    switch(kc) {
        ...
        case KEY_TO_TOGGLE_WINDOW_SIZE:
            if(current_graphics_mode == WINDOWED_MODE) {
                ...
                printf(" GRAPHICS ENGINE: Current scale factor: x%d\n", 
                       current_scale_factor); 
                destroy_all_textures();
                build_window_and_renderer();
                create_all_textures();
    ...
    // Call function that has been set up by user
    (*action_pointer_keyboard[e.key.keysym.scancode]) (START_ACTION);
}
